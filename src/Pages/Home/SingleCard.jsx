...
const SingleCard = ({ service }) => {
  ...
  return (
    <div>
      <div className="card bg-[#49adc0] dark:bg-cyan-700 text-white  w-10/12 mx-auto shadow-xl animated-card">
        <figure>
          <img className="w-full h-[250px]" src={image} alt="Shoes" />
        </figure>
        <div className="card-body">
          <h2 className="card-title  p-1 text-center rounded-md font-bold">
            {title}
          </h2>
          <p>{description}</p>
          <div className="card-actions justify-end">
            <Link to={`/feedback/${_id}`}>
              <div className="badge badge-outline btn dark:bg-cyan-500">See Details</div>
            </Link>
          </div>
        </div>
      </div>
